...
def find_true_void(stat):
    ...
    for i in range(11):
        if stat[0][i] != '.':
            ...
            if stat[0][pos_a:pos_b] == tuple(['.' for _ in range(abs(i-align[room_dst]))]):
                push_depth = can_push(stat[room_dst], room_dst)
                if push_depth != -1:
                    new_stat = [[a for a in stat[0]], [a for a in stat[1]], [a for a in stat[2]], [a for a in stat[3]], [a for a in stat[4]]]
                    ...
                    stat_queue[move_cost].append(tuple([tuple([a for a in b]) for b in new_stat]))
            ...
            if pop_depth >= 0:
                ...
                for h in range(align[i]-1,-1,-1):
                    if valid[h]:
                        if stat[0][h] == '.':
                            new_stat = [[a for a in stat[0]], [a for a in stat[1]], [a for a in stat[2]], [a for a in stat[3]], [a for a in stat[4]]]
                            move_cost = cost[stat[i][pop_depth]] * (1+pop_depth+abs(align[i]-h))
